...
public:
                ...
                Eigen::PlainObjectBase<DerivedFaces> &faces)
  {
	...
	Eigen::MatrixXd vertice_test;
	Eigen::MatrixXi faces_test;
    if(x_res <2 || y_res<2 ||z_res<2)
		return;
	//faces.resize(10000,3);
	faces_test.resize(10000000,3);
    int num_faces = 0;

	//vertices.resize(10000,3);
	vertice_test.resize(10000000,3);
    int num_vertices = 0;
    ...
    for (unsigned cube_it =0 ; cube_it < n_cubes; ++cube_it)
    {
	  ...
	  add_cube(values, points, isovalue, corner, vertice_test, num_vertices, faces_test, num_faces, edge2vertex);
	}
	...
	{
		vertices.row(i).x() = vertice_test.row(i).x();
		vertices.row(i).y() = vertice_test.row(i).y();
		vertices.row(i).z() = vertice_test.row(i).z();
	}
	for (int i = 0; i < num_faces; i++)
	{
		faces.row(i).x() = faces_test.row(i).x();
		faces.row(i).y() = faces_test.row(i).y();
		faces.row(i).z() = faces_test.row(i).z();
	}
